...
static void smart_ota_progress_callback(wifi_ota_status_t ota_status, uint32_t progress_percent);
static void smart_ota_complete_callback(wifi_ota_error_t error);

// Internal OTA functions (moved from simple_ota)
static sl_status_t smart_ota_start_download_with_callbacks(wifi_ota_progress_callback_t progress_cb,
                                                          wifi_ota_complete_callback_t complete_cb)
{
    ...
    app_log_info("OTA download started successfully\r\n");
    return SL_STATUS_OK;
}
...
void smart_ota_version_check_callback(version_check_error_t error, firmware_version_t remote_ver, version_compare_result_t result)
{
    ...
    switch (result) {
        case VERSION_NEWER:
            ...
            sl_status_t ota_status = smart_ota_start_download_with_callbacks(smart_ota_progress_callback, smart_ota_complete_callback);
...
sl_status_t smart_ota_force_update(void)
{
    ...
    sl_status_t status = smart_ota_start_download_with_callbacks(smart_ota_progress_callback, smart_ota_complete_callback);
    if (status == SL_STATUS_OK) {
        smart_ota_update_status(SMART_OTA_DOWNLOADING, 0);
    } else {
...
void smart_ota_default_version_callback(firmware_version_t current, firmware_version_t remote, version_compare_result_t result)
{
    ...
    app_log_info("Current version: %d.%d.%d\r\n", current.major, current.minor, current.patch);
    app_log_info("Remote version:  %d.%d.%d\r\n", remote.major, remote.minor, remote.patch);
