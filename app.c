...
// LOG_HEAP_DELTA(6);
  ...
  app_log_info("\\r\\n=== Smart OTA: Automatic Version Check and Update ===\\r\\n");
  osDelay(3000); // Wait 3 seconds for WiFi to stabilize

  // Initialize and start smart OTA system
  sl_status_t smart_status = smart_ota_init();
  if (smart_status == SL_STATUS_OK) {
    app_log_info("Smart OTA initialized successfully\\r\\n");

    // Start automatic version check and update
    smart_status = smart_ota_check_and_update();
    if (smart_status == SL_STATUS_OK) {
      app_log_info("Smart OTA check started successfully\\r\\n");
    } else {
      app_log_error("Smart OTA check failed: 0x%lx\\r\\n", smart_status);
    }
  } else {
    app_log_error("Smart OTA initialization failed: 0x%lx\\r\\n", smart_status);
  }
